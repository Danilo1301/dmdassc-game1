Button = class {
  ...
  OnMouseState(state) {
    ...
    if(state == MOUSE_STATE.UP) {
      if(this.selected && this.onClick_fn && this.isMouseOver) {
        this.onClick_fn();
      }
      this.selected = false;
    }
  ...
  Update() {
    this.isMouseOver = isPointInsideRect(Input.mouse.position, {x: this.position.x, y: this.position.y, width: this.width, height: this.height});
  }
